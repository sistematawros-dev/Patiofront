...
class Dashboard {
    ...
    setupMobileMenu() {
        ...
        // Função para recalcular alturas dos menus abertos
        const recalculateMenuHeights = () => {
            // Primeiro, recalcular nav-subsubmenu (menus mais internos)
            const openSubdropdowns = sidebar.querySelectorAll('.nav-subdropdown.open');
            openSubdropdowns.forEach(subdropdown => {
                const subsubmenu = subdropdown.nextElementSibling;
                if (subsubmenu && subsubmenu.classList.contains('nav-subsubmenu')) {
                    // Remover max-height temporariamente para obter altura real
                    subsubmenu.style.maxHeight = 'none';
                    const realHeight = subsubmenu.scrollHeight;
                    subsubmenu.style.maxHeight = '0';
                    // Forçar reflow
                    subsubmenu.offsetHeight;
                    // Aplicar altura real
                    subsubmenu.style.maxHeight = realHeight + 'px';
                }
            });

            // Depois, recalcular nav-submenu (menus pais) com um pequeno delay
            setTimeout(() => {
            ...
            // Terceira passagem para garantir que tudo está calculado corretamente
            setTimeout(() => {
                const openDropdowns = sidebar.querySelectorAll('.nav-dropdown.open');
                openDropdowns.forEach(dropdown => {
                    const submenu = dropdown.nextElementSibling;
        ...
        if (menuToggle) {
            menuToggle.addEventListener('click', (e) => {
                ...
                if (window.innerWidth > 768) {
                    ...
                    // Quando expandir, recalcular alturas
                    ...
                    // Quando abrir em mobile, recalcular alturas
                    if (!wasOpen) {
                        setTimeout(() => {
                            recalculateMenuHeights();
                        }, 600);
        ...
        window.addEventListener('resize', () => {
            if (window.innerWidth > 768) {
                sidebar.classList.remove('open');
                overlay.classList.remove('active');
                // Recalcular alturas após resize
                setTimeout(() => {
                    recalculateMenuHeights();
                }, 200);
            } else {
        ...
        // Recalcular alturas na inicialização
        setTimeout(() => {
            recalculateMenuHeights();
        }, 200);
    }
    ...
    setupDropdowns() {
        ...
        navDropdowns.forEach(dropdown => {
            ...
            dropdown.addEventListener('click', (e) => {
                ...
                if (submenu && submenu.classList.contains('nav-submenu')) {
                    if (!isOpen) {
                        // Abrir - calcular altura real imediatamente
                        const realHeight = submenu.scrollHeight;
                        submenu.style.maxHeight = realHeight + 'px';
                        submenu.style.opacity = '1';

                        // Recalcular após a animação para garantir altura correta
                        setTimeout(() => {
                            submenu.style.maxHeight = submenu.scrollHeight + 'px';
                        }, 450);
                    } else {
            ...
            // Auto-abrir dropdown pai se houver um item ativo no submenu
            if (submenu && submenu.querySelector('.nav-subitem.active, .nav-subsubitem.active')) {
        ...
        // Subdropdown da navegação (nested submenu)
        const navSubdropdowns = document.querySelectorAll('.nav-subdropdown');

        // Função auxiliar para recalcular altura do menu pai
        const recalculateParentHeight = (subdropdown) => {
            const parentNavGroup = subdropdown.closest('.nav-group');
            if (parentNavGroup) {
                const parentDropdown = parentNavGroup.querySelector('.nav-dropdown');
                const parentSubmenu = parentNavGroup.querySelector('.nav-submenu');

                if (parentDropdown && parentSubmenu && parentDropdown.classList.contains('open')) {
                    // Recalcular altura considerando o submenu expandido
                    // Múltiplas passagens para garantir que a altura está correta
                    setTimeout(() => {
        ...
        navSubdropdowns.forEach(subdropdown => {
            const subsubmenu = subdropdown.nextElementSibling;
            ...
            subdropdown.addEventListener('click', (e) => {
                ...
                if (subsubmenu && subsubmenu.classList.contains('nav-subsubmenu')) {
                    if (!isOpen) {
                        ...
                        // Recalcular altura do menu pai após expandir
                        recalculateParentHeight(subdropdown);

                        // Recalcular novamente após a animação
                        setTimeout(() => {
                        ...
                        // Recalcular altura do menu pai após recolher
                        setTimeout(() => {
                            recalculateParentHeight(subdropdown);
                        }, 50);
                    }
            ...
            // Auto-abrir submenu se houver um item ativo
            if (subsubmenu && subsubmenu.querySelector('.nav-subsubitem.active')) {
                subdropdown.classList.add('open');
                subsubmenu.style.maxHeight = subsubmenu.scrollHeight + 'px';
                subsubmenu.style.opacity = '1';

                // Também abrir o dropdown pai (Cadastros)
                const parentNavGroup = subdropdown.closest('.nav-group');
                if (parentNavGroup) {
                    const parentDropdown = parentNavGroup.querySelector('.nav-dropdown');
                    const parentSubmenu = parentNavGroup.querySelector('.nav-submenu');

                    if (parentDropdown && parentSubmenu) {
                        parentDropdown.classList.add('open');
                        parentSubmenu.style.opacity = '1';

                        // Calcular altura após o subsubmenu estar expandido
                        setTimeout(() => {
                            parentSubmenu.style.maxHeight = parentSubmenu.scrollHeight + 'px';
                        }, 10);
                    }
