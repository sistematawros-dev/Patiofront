...
document.addEventListener('DOMContentLoaded', function() {
    ...
    subTabs.forEach(tab => {
        tab.addEventListener('click', function(e) {
            ...
            // Se o href for apenas uma âncora (#), prevenir navegação
            // Caso contrário, permitir navegação normal
            if (href && href.startsWith('#')) {
                e.preventDefault();

                // Remover classe active de todos os sub-tabs
                subTabs.forEach(t => t.classList.remove('active'));

                // Adicionar classe active ao sub-tab clicado
                this.classList.add('active');

                // Aqui você pode carregar conteúdo diferente baseado no sub-tab
                const tabName = this.textContent.trim();
                console.log('Sub-tab selecionada:', tabName);

                // Exemplo: atualizar título da página
                const pageTitle = document.querySelector('.page-title');
                if (pageTitle) {
                    pageTitle.textContent = tabName;
                }
            }
            // Se for um link real (./grupo-empresa.html, etc), deixar navegar normalmente
        });
    });
